...
export default function App() {
    ...
    const initializeConfiguration = async () => {
      try {
        ...
        if (memoryMonitorRef.current) {
          activeIntervals.current.add(memoryMonitorRef.current);
        }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      activeIntervals.current.forEach((interval: NodeJS.Timeout) => clearInterval(interval));
      activeTimeouts.current.forEach((timeout: NodeJS.Timeout) => clearTimeout(timeout));
      activeIntervals.current.clear();
      activeTimeouts.current.clear();
  ...
  const cleanupAllOperations = useCallback(() => {
    try {
      ...
      activeIntervals.current.forEach((interval: NodeJS.Timeout) => {
        clearInterval(interval);
      });
      activeIntervals.current.clear();
      
      // Clear all active timeouts
      activeTimeouts.current.forEach((timeout: NodeJS.Timeout) => {
