    ...
    <div class="receipt-container">
            ...
            <div class="column">
                <div class="transaction-details">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">{{ ucfirst($transaction->rental_type) }}</span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">Rental Date:</span>
                        <span class="detail-value">{{ \Carbon\Carbon::parse($transaction->rental_date)->format('M d, Y') }}</span>
                    </div>
                    @if($transaction->return_date)
                    <div class="detail-row">
                        <span class="detail-label">Return Date:</span>
                        <span class="detail-value">{{ \Carbon\Carbon::parse($transaction->return_date)->format('M d, Y') }}</span>
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div class="amount-section">
            <div class="detail-row">
                ...
                <span class="detail-value">{{ $peso_symbol }}{{ number_format($transaction->quotation_amount, 2, '.', ',') }}</span>
            </div>
            @if($transaction->total_penalties && $transaction->total_penalties > 0)
            <div class="detail-row">
                <span class="detail-label">Penalties:</span>
                <span class="detail-value">{{ $peso_symbol }}{{ number_format($transaction->total_penalties, 2, '.', ',') }}</span>
            </div>
            @endif
            <div class="total-amount">
                Total: {{ $peso_symbol }}{{ number_format($transaction->quotation_amount + ($transaction->total_penalties ?? 0), 2, '.', ',') }}
            </div>
        </div>
