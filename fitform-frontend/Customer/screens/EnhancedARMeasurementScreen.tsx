...
import { arMeasurementService, BodyMeasurements } from '../../services/ARMeasurementService';
import { ProportionalMeasurementsCalculator } from '../../src/utils/ProportionalMeasurements';
import { NavigationUtils } from '../../src/utils/NavigationUtils';
import { SimpleNavigation } from '../../src/utils/SimpleNavigation';
import { NavigationErrorBoundary } from '../../src/components/NavigationErrorBoundary';

// Import AR components with error handling
let ARSessionManager: any = null;
try {
  ARSessionManager = require('../../src/ARSessionManager').default;
} catch (error) {
  console.log('⚠️ ARSessionManager not available in development mode');
}

const { width, height } = Dimensions.get('window');

interface LocalBodyMeasurements {
  height: number;
...
export default function EnhancedARMeasurementScreen() {
  // Use navigation hooks directly - React will handle the errors
  ...
  const [mockMeasurements, setMockMeasurements] = useState<LocalBodyMeasurements | null>(null);
  ...
  useEffect(() => {
    const checkDevelopment = () => {
      ...
      const hasNativeModules = (global as any).nativeCallSyncHook !== undefined;
  ...
  const initializeARSession = useCallback(async () => {
    try {
      if (isDevelopment) {
        ...
        console.log('📱 To test real AR: Build development version and install on physical device');
      ...
      if (!ARSessionManager) {
        throw new Error('ARSessionManager not available');
      }

      const arManager = new ARSessionManager();
      
      // Check AR support
      const isSupported = await arManager.isARSupported();
      if (!isSupported) {
      ...
      setSessionState(prev => ({ 
        ...prev, 
        error: (error as Error).message || 'Failed to initialize AR session',
        isActive: false 
      }));
      return false;
    }
  ...
  const startBodyScan = useCallback(async () => {
    ...
    const progressTimer = setInterval(() => {
      ...
      if (progress >= 50) {
        setSessionState(prev => {
          if (prev.currentScan === 'front') {
            return { ...prev, currentScan: 'side' };
          }
          return prev;
        });
      }
  ...
  const completeScan = useCallback(async () => {
    ...
    if (isDevelopment || scanAttempts >= 1) {
      ...
      const mockData: LocalBodyMeasurements = {
        height: proportionalMeasurements.height,
        shoulderWidth: proportionalMeasurements.shoulderWidth,
        ...
        timestamp: proportionalMeasurements.timestamp
      };
      setMockMeasurements(mockData);
      setSessionState(prev => ({ 
        ...prev, 
        isScanning: false,
        ...
        measurements: {
          ...mockData,
          shoulder_width: mockData.shoulderWidth,
          scan_type: 'ar'
        },
        scanProgress: 100
      }));
      ...
      Alert.alert(
        'Scan Complete!', 
        `Body measurements detected:\nHeight: ${mockData.height.toFixed(1)}cm\nShoulder: ${mockData.shoulderWidth.toFixed(1)}cm\nChest: ${mockData.chest.toFixed(1)}cm\nWaist: ${mockData.waist.toFixed(1)}cm\nHips: ${mockData.hips.toFixed(1)}cm\nConfidence: ${(mockData.confidence * 100).toFixed(1)}%`
      );
    } else {
    ...
    return (
      <View style={styles.measurementsContainer}>
        ...
        <View style={styles.measurementRow}>
          ...
          <Text style={styles.measurementValue}>{measurements.shoulder_width.toFixed(1)} cm</Text>
            </View>
        <View style={styles.measurementRow}>
        ...
        <TouchableOpacity 
          onPress={() => {
            try {
              ...
              console.log('NavigationUtils failed, trying SimpleNavigation:', (error as Error).message);
